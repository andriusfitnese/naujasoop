...
void atrinkimas2(Vector<Stud>& grupe, Vector<Stud>& galiorka, int pasmv, duration<double>& veiklaik)
{
	...
	grupe.erase(std::remove_if(grupe.begin(), grupe.end(), [](const Stud& s) { return s.getGalutinis() < 5; }), grupe.end());
	auto end1 = high_resolution_clock::now();
	veiklaik += end1 - start1;
...
void atrinkimas3(Vector<Stud>& grupe, Vector<Stud>& galiorka, int pasmv, duration<double>& veiklaik)
{
	...
	auto start1 = high_resolution_clock::now();
	auto it = std::partition(grupe.begin(), grupe.end(), [](const Stud& s) { return s.getGalutinis() >= 5; });
	std::copy(it, grupe.end(), back_inserter(galiorka));
